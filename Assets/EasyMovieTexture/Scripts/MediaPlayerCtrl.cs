...
public class MediaPlayerCtrl : MonoBehaviour {
...
#else
				Call_SetUnityTexture(m_VideoTexture.GetNativeTexturePtr().GetHashCode());
#endif
				#endif
				Call_SetWindowSize();
				m_bCheckFBO = true;
	...
	IEnumerator DownloadStreamingVideoAndLoad(string strURL)
	{
		...
		UnityWebRequest www = new UnityWebRequest(strURL);
    ...
    IEnumerator CopyStreamingAssetVideoAndLoad(string strURL)
	{
		...
		if(System.IO.File.Exists(write_path) == false)
		{
			...
			using (UnityWebRequest www = UnityWebRequest.Get(Application.streamingAssetsPath + "/" + strURL))
			{
